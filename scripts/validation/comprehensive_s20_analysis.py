"""
Comprehensive analysis of s20above.xlsx validation set
...
def load_all_papers():
    """Load ALL papers from s20above.xlsx validation file with corpus abstracts"""
    print("\nLoading ALL validation labels from s20above.xlsx...")
    s20_path = project_dir / "data" / "input" / "s20above.xlsx"
    labels_df = pd.read_excel(s20_path)
    
    print(f"Total papers in s20above.xlsx: {len(labels_df)}")
    print("\nLabel distribution:")
...
def test_all_papers():
    ...
    print("="*80)
    print(f"TESTING ALL {total} PAPERS FROM s20above.xlsx")
    print("="*80)
    ...
    results_file = project_dir / "scripts" / "validation" / "s20_comprehensive_results.csv"
